...
describe('Random root seed generation', () => {
  it('From 256 bit entropy / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().length, 32);
    assert.strictEqual(typeof rootSeed.getMnemonicSentence(), 'string');
    assert.strictEqual(rootSeed.getMnemonicSentence().split(' ').length, 24);
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().length, 64);
  });
  ...
  it('From 224 bit entropy / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().length, 28);
    assert.strictEqual(typeof rootSeed.getMnemonicSentence(), 'string');
    assert.strictEqual(rootSeed.getMnemonicSentence().split(' ').length, 21);
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().length, 64);
  });
  ...
  it('From 192 bit entropy / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().length, 24);
    assert.strictEqual(typeof rootSeed.getMnemonicSentence(), 'string');
    assert.strictEqual(rootSeed.getMnemonicSentence().split(' ').length, 18);
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().length, 64);
  });
  ...
  it('From 160 bit entropy / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().length, 20);
    assert.strictEqual(typeof rootSeed.getMnemonicSentence(), 'string');
    assert.strictEqual(rootSeed.getMnemonicSentence().split(' ').length, 15);
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().length, 64);
  });
  ...
  it('From 128 bit entropy / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().length, 16);
    assert.strictEqual(typeof rootSeed.getMnemonicSentence(), 'string');
    assert.strictEqual(rootSeed.getMnemonicSentence().split(' ').length, 12);
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().length, 64);
  });
...
describe('Root seed loading', () => {
  it('From 24 words mnemonic sentence / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), 'aca4318d552bd2119aee96b09c849bab97b36a9485dbc31fa09a76aecc5e3528');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'protect canoe globe prevent rule angle hill spot rack tomorrow cheap fire kite relief faith rocket seek wonder charge unhappy under congress start brick');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), 'a2bf4321dd99b58c996f060c23d5584865d96d1d836944d043c537a546546663d2748a601c8df3577ff5752dcd92496893edc14246732b930f080685ef0ecc25');
  });
  it('From 24 words mnemonic sentence / With password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), '170d60b4d4daa95df3181daa69c05289ed40ce93383d5db8b843c25fbdef7f14');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'blade help coffee predict pride puzzle small admit price evoke believe because staff guard erupt amateur frozen shift axis lucky wink waste wisdom mechanic');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), 'f2cabc07c15edde0f6c22323159fb89213cc0b7bc2c02d771ed531f1fda72bafbc7476d63feb83f328a5e0d35096e3081289d9d56a7924eda8461385157f221a');
  });
  it('From 21 words mnemonic sentence / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), '0e4e238bc08a6c7a108e966aa7802763f5fb57891bca2947b9289e49');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'atom illegal title library plug dial drama spot height despair antenna side garden profit caught junk citizen differ enforce develop ostrich');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), '711588bf80eb4be71448486c0106b86dbc37772392e293da5737356a37d3ac8fce74bb2335e1b312822e89019f25ee48589e45cddd3fb71b00cad036b8c8ddf0');
  });
  it('From 21 words mnemonic sentence / With password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), '4f9fbba127f2df0e37218340d227c3459bf7b2a0eaa287f5eb2f7312');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'exhibit worry tribe exit combine mandate symptom corn dolphin muffin valid menu satoshi sun lonely february margin quantum slim tower chest');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), '6fb660cba9a8788095804b7751cb21cf91ecf3b1b1ee19faa1b65317314021eee7321171d97b628239d3b84d06913cc5730076b699ad928355c5f4e874788807');
  });
  it('From 18 words mnemonic sentence / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), '4439f76c150058345b30bf48801ef5229177cbf65b500015');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'during soul swallow claw airport boss holiday blood embrace abstract run early blast tourist sunny head abandon praise');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), '66187f3094191fbd6f62b1e8e81e50003d73ae866d4696f6b48373c3150c8dec258138bb6f4c3601b83a6d0d3e522399a516befdb2329bd52c90baec989c40ec');
  });
  it('From 18 words mnemonic sentence / With password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), 'b9d59b5cd5096b0677a39aa53d16a546b79f614c820bf9ac');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'right proud strong present note lobster teach infant pioneer trim power miracle ketchup radar craft door whale gate');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), '9f89de31c59dd37ba8674abadbc74cddcde9840bd2b1fea479c4327c67322d9d3c47025f6ccb94d7419498e97e9b502e5ad53f199b9a533b90b2e787327ae715');
  });
  it('From 15 words mnemonic sentence / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), '853f2f8c3e5afae2904907a9ebb0a66a0a0cf4e7');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'lunch west toast lake quick image donkey can prepare frown civil staff patch kidney paper');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), 'be054fad85b8ab8ee58af4a2755e15f709ee8ec223b34dc41c7670aec437a34c7ee4409380864a6b7d486e175d52d3b5b5d81d570e3a816ebb75caddfa1c4fca');
  });
  it('From 15 words mnemonic sentence / With password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), '55a831b12e73187fd2cc9339a691700a377dd850');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'fiber dose hobby friend cover divide enroll mystery defy cruel foster behave jelly umbrella drop');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), '9ef0386bd11c47495ffa9e74fbd55f82d5af8b8e1c39337b5e52fb91ca57e6007321532eeb3d6113eb9d82c3161ec3bb59792d95639bda7b6c1014c936ed43b2');
  });
  it('From 12 words mnemonic sentence / Without password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), '24e33abbc6c09085ab8158388b288b77');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'ceiling border puzzle mirror animal drift puppy climb decade flock carpet upset');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), '2329d884afd13c82bd951bc0a382964f469d2dc4aeaeee6c4f8ffb46635ace25b4a49f5edefee10fef794306055ec5adf79a551b158814fae432c3c5e3846b72');
  });
  it('From 12 words mnemonic sentence / With password', () => {
    ...
    assert.strictEqual(typeof rootSeed.getEntropy(), 'object');
    assert.strictEqual(rootSeed.getEntropy() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getEntropy().toString('hex'), '20e8321dd93b198da3faf75c0eea9b8a');
    assert.strictEqual(rootSeed.getMnemonicSentence(), 'can dose manual raven ranch short morning galaxy foster jar plunge betray');
    assert.strictEqual(typeof rootSeed.getValue(), 'object');
    assert.strictEqual(rootSeed.getValue() instanceof Buffer, true);
    assert.strictEqual(rootSeed.getValue().toString('hex'), '4be26f5c72d3fd5a591e7e31c9c1d7dd30e963408fb89231461856cfdf555d948114094be2239583c3b6aa541a856959cff8137f711b78ade46c02130ad33d23');
  });
});
