...
class RootSeed {
  constructor(input, password) {
    ...
    if (typeof input === 'string') {
      ...
      this.value = RootSeed.convertMnemonicSentenceToSeed(this.mnemonicSentence, rootSeedPassword);
      this.validateMnemonicSentence();
  ...
  static encodeMnemonicSentence(data) {
    ...
    while (bitsEncoded < bitsToEncode) {
      /* eslint-disable no-restricted-properties */
      /* eslint-disable max-len */
      const wordListIndex = Math.floor(((paddedData.readUInt32BE(Math.floor(bitsEncoded / 8)) * Math.pow(2, bitsEncoded % 8)) % 4294967296) / Math.pow(2, 21));
      /* eslint-enable no-restricted-properties */
      /* eslint-enable max-len */
